...
CREATE TABLE ausleihe (
    ...
    anzahl_tage INTEGER CHECK (anzahl_tage > 0)
);
...
-- Man kann beim 1ten INDEX schnell Lesernamen suchen, diese werden oft gesucht deshalb sinnvoll
-- Ebenso beim 2ten, es werden sehr oft Bücher gesucht, deshalb sinnvoll --> will wissen, ist das Buch im Besitz von jemandem? 
-- Hat man es überhaupt?
-- Das 3te ist sinnnvoll um zu sehen, welche Bücher alle ausgeliehen sind und 
-- welche vielleicht schon anstehen über dem rueckgabe datum
-- DEIN CODE HIER:
CREATE INDEX leser_inx ON leser (name);
CREATE INDEX buch_inx ON buch (titel);
CREATE INDEX ausleih_inx ON ausleih (rueckgabe_datum);
...
--
-- DEIN CODE HIER:
    INSERT INTO ausleihe(leser_id, buch_id, ausleih_datum, rueckgabe_datum) VALUES (1, 1, "2024-01-10", "2024-01-24");
    INSERT INTO ausleihe(leser_id, buch_id, ausleih_datum, rueckgabe_datum) VALUES (1, 14, "2024-02-01", NULL);
    INSERT INTO ausleihe(leser_id, buch_id, ausleih_datum, rueckgabe_datum) VALUES (2, 1, "2024-01-15", "2024-01-22");
    INSERT INTO ausleihe(leser_id, buch_id, ausleih_datum, rueckgabe_datum) VALUES (2, 1, "2024-02-05", NULL);
    INSERT INTO ausleihe(leser_id, buch_id, ausleih_datum, rueckgabe_datum) VALUES (3, 1, "2024-01-20", "2024-01-25");
...
SELECT b.titel AS buch_titel, l.name AS leser_name, a.ausleih_datum AS seit
FROM ausleihe a
JOIN leser l ON a.leser_id = l.id
JOIN buch b ON a.buch_id = b.id
...
SELECT l.name, COUNT(*) AS gesamt_ausleihen
FROM ausleihe a
JOIN leser l ON a.leser_id = l.id
GROUP BY l.name
ORDER BY gesamt_ausleihen DESC
LIMIT 1;
